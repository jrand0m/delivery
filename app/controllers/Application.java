...
public class Application extends Controller
{
	...
	public static void _prepare()
	{
		...
		if( user != null )
		{
			Order order = Order.all( Order.class ).filter( "deleted", Boolean.FALSE )
	...
	private static User getCurrentUser()
	{
		...
		{
			List<User> users = User.all( User.class ).filter( "login", Security.connected() ).fetch();
			if( users.size() != 0 )
			{
				user = users.get( 0 );
			}
	...
	public static void showMenu( Long id )
	{
		...
		List<MenuItem> menuItems = Model.all( MenuItem.class ).filter( "client", client ).fetch();
	...
	public static void delOrderItem( Long id )
	{
		if( Security.isConnected() )
		{
			...
			OrderItem orderitem = Model.all( OrderItem.class ).filter( "orderId", order ).filter( "deleted", false )
