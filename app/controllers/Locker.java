...
public class Locker extends Controller
{
	...
	public static void index()
	{
		...
		List<User> users = Model.all( User.class ).filter( "login", userName ).fetch();
		if( users.size() == 0 )
		{
			forbidden();
		}
		User user = users.get( 0 );
		List<Address> addressList = Model.all( Address.class ).filter( "deleted", false ).filter( "userId", user )
				.fetch();
	...
	public static void basket()
	{
		...
		User user = users.get( 0 );
		Order order = Model.all( Order.class ).filter( "orderOwner", user ).filter( "orderStatus", OrderStatus.OPEN )
				.get();
	...
	public static void addAddress( Address address )
	{
		...
		String userName = Security.connected();
		List<User> user = Model.all( User.class ).filter( "login", userName ).fetch( 1 );
		if( user.size() != 1 )
		{
			forbidden();
		}
