...
public interface OrderMapper {
    ...
    @Select("insert into vd_order (confirmed_courier_id, delivery_address_id, order_owner_id, restaurant_id, \"declineMessage\", deleted," +
            ...
            "  #{deliveryPrice}::money,  #{orderAccepted},  #{orderClosed},  #{orderConfirmed},  #{orderCooked},  #{orderCreated}, #{orderDelivered}," +
            "  #{orderPlanedCooked}::interval, #{orderPlanedDeliveryTime}::interval, #{orderStatus}, #{orderTaken}, #{paymentStatus}, #{totalMenuPrice}::money)" +
            " returning vd_order.*, regexp_replace(\"deliveryPrice\"::text, '[$]', \"deliveryPrice_currency\"||' ' , 'g') as \"deliveryPrice\", " +
