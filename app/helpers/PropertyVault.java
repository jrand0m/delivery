...
public class PropertyVault {
	...
	public static String getSystemValueFor(String key) {
		String value = null;
		SystemSetting obj = SystemSetting.find(
				SystemSetting.FIELDS.SYSTEMSETTING_stg_key + " = ? and "
						+ SystemSetting.FIELDS.SYSTEMSETTING_ISDEFAULT_SETTING
						+ " = ? ", key, false).first();
		if (obj != null) {
			value = obj.stg_value;
		} else {
			obj = SystemSetting
					.find(SystemSetting.FIELDS.SYSTEMSETTING_stg_key
							+ "= ? and "
							+ SystemSetting.FIELDS.SYSTEMSETTING_ISDEFAULT_SETTING
							+ " = ?", key, true).first();
			if (obj != null) {
